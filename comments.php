...
<div id="comments" class="comments my-4">
    ...
    <h3 class="mb-5">Комментарии:</h3>
		...
		<ol class="comment-list p-0">
          ...
          'style'             => 'ol', // во что оборачивать комменты
          'callback'          => null, // какая функция будет выводить контент =>
          'end-callback'      => null, // конец
          'type'              => 'all',
		...
		if ( ! comments_open() ) :
			?>
			<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'example' ); ?></p>
			<?php
		endif;
  ...
  $defaults = [
    ...
    'comment_field'        => '<div class="comment-form-comment mb-3">
      <textarea id="comment" name="comment" class = "form-control" cols="45" rows="8"  aria-required="true" required="required" placeholder = "Коментарий"></textarea>
    </div>',
    'must_log_in'          => '<p class="must-log-in">' .
       sprintf( __( 'Вам нужно <a href="%s">logged in</a> чтобы оставить комментарий.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post->ID ) ) ) ) . '
     </p>',
