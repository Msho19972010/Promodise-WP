...
if (!function_exists('study_theme_setup')) {
  function study_theme_setup()
  {
    ...
    // switch on post's thumbnails
    add_theme_support('post-thumbnails');
    set_post_thumbnail_size( 730, 480, true );
    // switch on html5 tags
...
function study_theme_scripts()
{
  ...
  // Icofont Css
  wp_enqueue_style('all_css', get_template_directory_uri() . '/plugins/fontawesome/css/all.css', array('main'), null);
  // Animation
  wp_enqueue_style('animation', get_template_directory_uri() . '/plugins/animate-css/animate.css', array('main'), null);
  // Icofont
  wp_enqueue_style('icofont', get_template_directory_uri() . '/plugins/icofont/icofont.css', array('main'), null);

  // Main style-sheet
  wp_enqueue_style('study', get_template_directory_uri() . '/css/style.css', array('main'), '1.0.11');
  ...
  wp_register_script('jquery', get_template_directory_uri() . '/plugins/jquery/jquery.min.js', '1.0.7');
  ...
  // Woow animation
  wp_enqueue_script('woow', get_template_directory_uri() . '/plugins/counterup/wow.min.js', array('jquery'), '1.0.0', true);
  ...
  // Contanc form
  wp_enqueue_script('contact', get_template_directory_uri() . '/plugins/form/contact.js', array('jquery'), '1.0.2', true);
  wp_enqueue_script('custom', get_template_directory_uri() . '/js/custom.js', array('jquery'), '1.0.13', true);
}
...
add_action('after_setup_theme', function () {
  register_nav_menus([
    ...
    'footer_menu_left' => 'Footer Left menu',
    'footer_menu_right' => 'Footer Right menu'
  ]);
});
...
function test_widgets_init () {
  ...
  register_sidebar( array(
    'name'          => esc_html__('Сайдбар блога', 'test_theme'),
    'id'            => "sidebar-blog",
  ...
  // footer text-sidebar
  register_sidebar( array(
    'name'          => esc_html__('Сайдбар в подвале', 'test_theme'),
  ...
  // footer contacts-sidebar
  register_sidebar( array(
    'name'          => esc_html__('Контакты в подвале', 'test_theme'),
    'id'            => "sidebar-footer-contacts",
    'before_widget' => '<div id="%1$s" class="footer-widget footer-text %2$s">',
    'after_widget'  => '</div>',
    'before_title'  => '<h4>',
    'after_title'   => '</h4>'
  ) );
...
add_filter( 'pre_render_block', function( $pre_render_block, $args ) {
  ...
  $form = '<form class="form-group" id="searchform" method="get" role="search" action="' . home_url('/') .'">
              <input class="form-control" type="text" placeholder="Поиск" value="' . get_search_query() . '" name="s" id="s">
...
class Download_widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			'Полезные файлы',
			array( 'description' => 'Прикрепите ссылки полезные файлы', 'classname' => 'download' )
		);
...
//Register new type of note - услуги
...
//Register new type of note - партнёры
  
  register_post_type('partner', array(
		'labels'             => array(
			'name'               => __('Партнеры'), // Основное название типа записи
			'singular_name'      => __('Партнёр'), // отдельное название записи типа partner
			'add_new'            => __('Добавить новго партнёра'),
			'add_new_item'       => __('Добавить новго партнёра'),
  ...
  //Register new type of note - Тарифы
  
  register_post_type('price', array(
		'labels'             => array(
			'name'               => __('Тарифы'), // Основное название типа записи
			'singular_name'      => __('Тариф'), // отдельное название записи типа partner
			'add_new'            => __('Добавить новый тариф'),
			'add_new_item'       => __('Добавить новый тариф'),
			'edit_item'          => __('Редактировать тариф'),
			'new_item'           => __('Новый Цена'),
  ...
  //Register new type of note - Отзывы
  
  register_post_type('testimonial', array(
		'labels'             => array(
			'name'               => __('Отзывы'), // Основное название типа записи
			'singular_name'      => __('Отзывы'), // отдельное название записи типа partner
...
function my_phpmailer_example( $phpmailer ) {
	...
	$phpmailer->Port = 25;
	$phpmailer->Username = 'info@co06607-wordpress-4k4st.tw1.ru';
	$phpmailer->Password = 'yFgb42L3';

	// Additional settings…
	//$phpmailer->SMTPSecure = "tls"; // Choose SSL or TLS, if necessary for your server
	$phpmailer->From = "info@co06607-wordpress-4k4st.tw1.ru";
	$phpmailer->FromName = "WP";
}
...
function my_action_callback() {
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    # Mail Content
    $subject ='From Wordpress';
    $content = "Name: $name\n";
    $content .= "Email: $email\n\n";
    $content .= "Message:\n$message\n";

    # email headers.
    $headers = "From: Wordpress <info@co06607-wordpress-4k4st.tw1.ru>";
...
// // add class 'nav-item' for all menu items

// add_filter('nav_menu_css_class', 'custom_nav_menu_css_class', 10, 1);

// // get classes of all menu items
// function custom_nav_menu_css_class($classes)
// {
//   // add to list of classes our own class 'nav-item'
//   $classes[] = 'nav-item';
//   // return list of classes with our class
//   return $classes;
// }

// // add classes on links 'nav-link'
